                ...
                <div class="lg:w-[90rem] md:w-[40rem]  sm:w-[25rem] bg-white rounded-2xl px-6 py-5 mt-16 shadow-2xl">
                  <div class="flex items-center justify-between mb-5 p-5">
                    ...
                    <div class="flex items-center">
                      <button class="bg-blue-700 hover:bg-blue-900 text-white font-bold py-2 px-4 rounded-xl mr-3 flex">
                  ...
                  <div class="overflow-x-auto rounded-xl">
                    <table class="w-full text-sm text-left text-gray-500">
                      ...
                      <tbody>
                      <tr class="border-b-gray-100 border-b-2" v-for="employee in pagePointing" :key="employee.id">
                        <td class="text-gray-900 p-5">{{ employee.id }}</td>
                        <td class="text-gray-900">{{ employee.employee }}</td>
                        <td>{{ employee.role }}</td>
                        <td>{{ employee.department }}</td>
                        <td>{{ employee.date }}</td>
                        <td :class="checkColor(employee)">{{ employee.status }}</td>
                        <td :class="textColor(employee)">{{ employee.checkIn }}</td>
                        <td :class="textColor(employee)">{{ employee.checkOut }}</td>
                        <td :class="textColor(employee)">{{ employee.workHours }}</td>
                      </tr>
                      </tbody>
                    </table>
                  </div>
...
const searchpoint = (query: String) => {
  return pointing.filter( employee => employee.employee.toLowerCase().includes(recherche.value.toLowerCase())
...
const pointingFiltres = computed(() => {
  ...
  return searchpoint(recherche.value);
});
...
const pointingLocalStorage = localStorage.getItem('pointing');
...
const getCurrentPageData = () => {
  const currentPage = 1;
  const employees = JSON.parse(localStorage.getItem('pointing') || '[]');
